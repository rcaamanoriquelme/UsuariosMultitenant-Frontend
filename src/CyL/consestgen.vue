        ...
        >
            <template v-slot:top>
                ...
                <v-toolbar flat color="white">
                    <v-toolbar-title class="rdmqc">Estadísticas Generales de Entrada Primaria</v-toolbar-title>
                </v-toolbar>
            ...
            <template v-if="this.desserts.length > 0" v-slot:foot>
                ...
                <tr class="border-top border-primary border-bottom border-primary">
                    ...
                    <th class="pl-4">{{ objEstadisticas.totalMontoFormateado }}</th>
                </tr>
            </template>
        </v-data-table>
    </div>
...
import { getCadenas, getLocales, getEmisores, getLocalesByCadena, getDetalleByFiltro } from '../services/ConsestgenService';
import Vue from 'vue';
...
export default {
    ...
    data(){
        return {
            ...
            selected: [],
            tableHeaders: [
            { text: "Emisor", value: "emisornombre", width: "50%" },
            { text: "N° trx Recibidas", value: "cantidadtrx", width: "25%" },
            { text: "Monto "+config.symbolMoney, value: "montoFormateado", width: "25%" },
            ],
            objEstadisticas: {
                totaltrxrecibidas: "",
                totalmontotrx: ""
            ...
            if (this.startDate && this.endDate) {
                ...
                try {
                    ...
                    if (estadisticas.detalleemisortransacciones.length != 0) {
                        ...
                        this.objEstadisticas.totalMontoFormateado = estadisticas.totalMontoFormateado;
        ...
        exportarPlanilla() {
            ...
            try {
                if (this.desserts.length > 0) {
                    ...
                    'Emisor': row.emisornombre,
                    'Recibidas': row.cantidadtrx,
                    ['Monto ' + config.symbolMoney]: row.montoFormateado,
                    }));
                    data.push({
                        'Emisor': 'Totales:',
                        'Recibidas': this.objEstadisticas.totaltrxrecibidas,
                        ['Monto ' + config.symbolMoney]: this.objEstadisticas.totalMontoFormateado
                    });
