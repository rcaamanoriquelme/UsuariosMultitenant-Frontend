        ...
        >
            <template v-slot:top>
                ...
                <v-toolbar flat color="white">
                    <v-toolbar-title class="rdmqc">Carta de Reclamo</v-toolbar-title>
                </v-toolbar>
...
import { getCadenas, getLocales, getLocalesByCadena, getEmisores, getOperadoresByEmisor, 
    getTarjetasByEmisor, getTipoTrxByEmisor, 
    getDetalleCartaDeReclamoByFiltro } from '../services/InfocarrecService';
import Vue from 'vue';
            ...
            // VALIDACIONES
                ...
                if (this.startDate && this.endDate) {
                    ...
                    const filtro = {
                        ...
                        id_trx: null, // DEPRECADO
                        f_fin: this.endDate,
                        vuelto: null, // DEPRECADO
                        ...
                        id_estado_trx: null, // DEPRECADO
                        estado_con: null, // DEPRECADO
                        estado_liq: null, // DEPRECADO
                        per_cambio_pago: null, // DEPRECADO
                        per_codigo_grupo_banco: null,
                        per_cuenta_contable: null,
                        per_car_operador: null, // DEPRECADO
                        per_motivo_anulacion: null, // DEPRECADO
                        id_operador: this.operadorFilterValue,
                        mpa_nombre: null, // DEPRECADO
                        es_vale: null, // DEPRECADO
                    ...
                    try {
                        const listaCartaDeReclamo = await getDetalleCartaDeReclamoByFiltro(filtro);
                            this.showInfoMsg({message: "Se encontraron " + listaCartaDeReclamo.length + " registros."});
                            if (listaCartaDeReclamo.length != 0) {
                                listaCartaDeReclamo.forEach((objCartaDeReclamo) => {
                                    if (objCartaDeReclamo.lan_cant == null) {
                                        objCartaDeReclamo.lan_cant = 0;
                                    }
                                    objCartaDeReclamo.trx_fecha_formateada = objCartaDeReclamo.trx_fecha + " " + objCartaDeReclamo.trx_hora;
                                    this.desserts.push(objCartaDeReclamo);
                                });
                            }
                    } catch (error) {
                        console.log(error);
        ...
        exportarPlanilla() {
            ...
            try {
                if (this.desserts.length > 0) {
                    ...
                    XLSX.utils.book_append_sheet(wb, ws, "Carta-de-Reclamo");
                    XLSX.writeFile(wb, "carta-de-reclamo.xlsx");
