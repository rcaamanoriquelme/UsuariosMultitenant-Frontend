<template>
    <div>
        ...
        <!-- DIALOG DE Log de archivos -->
        <v-dialog v-model="dialogLogArchivos" scrollable>
            <!-- CONFIGURACIÓN TABLA Log de archivos-->
            <v-data-table 
            v-model="selectedLogArchivos"
            :headers="tableHeadersLogArchivos"
            :items="dessertsLogArchivos"
            :loading="loadTableLogArchivos"
            loading-text="Cargando Log de archivos..."
            :items-per-page="10"
            ...
            <template v-slot:top>
                <v-toolbar flat color="white">
                    <v-toolbar-title class="rdmqc">Log de archivos</v-toolbar-title>
                </v-toolbar>
            </template>
        ...
        >
            ...
            <template v-slot:[`item.colRecibida`]="{ item }">
                <div v-if="item.trx_recibida_cant > 0">
                    <a v-bind:href="generarUrl(true, item.id_emisor, 'REC')">
            ...
            <template v-slot:[`item.colConciliada`]="{ item }">
                <div v-if="item.trx_conciliada_cant > 0">
                    <a v-bind:href="generarUrl(true, item.id_emisor, 'CON')">
            ...
            <template v-slot:[`item.colPendiente`]="{ item }">
                <div v-if="item.trx_pendiente_cant > 0">
                    <a v-bind:href="generarUrl(true, item.id_emisor, 'PEN')">
            ...
            <template v-slot:[`item.colPrelitigio`]="{ item }">
                <div v-if="item.trx_prelitigio_cant > 0">
                    <a v-bind:href="generarUrl(true, item.id_emisor, 'PRE,PRU')">
            ...
            <template v-slot:[`item.colLitigio`]="{ item }">
                <div v-if="item.trx_litigio_cant > 0">
                    <a v-bind:href="generarUrl(true, item.id_emisor, 'LIT')">
            ...
            <template v-slot:[`item.colDesconocida`]="{ item }">
                <div v-if="item.trx_desconocida_cant > 0">
                    <a v-bind:href="generarUrl(true, item.id_emisor, 'DES')">
            ...
            <template v-slot:[`item.colArchivos`]="{ item }">
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <div>
                    <div>{{ item.ult_arch_conc }}</div>
                    <div>{{ item.ult_arch_liq }}</div>
                    </div>
                    <v-icon 
                    small
                    @click="showDialogLogArchivos(item.id_emisor)">assignment</v-icon>
                </div>
            </template>
...
import { getCadenas, getDetalleByFiltro, getDetalleLogArchivoFiltro } from '../services/ConcmonconService';
import Vue from 'vue';
...
export default {
    ...
    data(){
        return {
            ...
            // MODAL LGO ARCHIVOS
            dialogLogArchivos: false,
            selectedLogArchivos: [],
            tableHeadersLogArchivos: [
                { text: "Fecha", value: "fecha", width: "15%"},
                { text: "Emisor", value: "emi_nombre", width: "15%"},
                { text: "Tipo", value: "lar_evento", width: "10%"},
                { text: "Lote", value: "lar_lote", width: "10%"},
                { text: "Archivo", value: "lar_archivo", width: "15%"},
                { text: "Descripción", value: "lar_descripcion", width: "35%"},
            ],
            dessertsLogArchivos: [],
            loadTableLogArchivos: false,
    ...
    methods: {
        ...
        async detalleByFiltro() {
            ...
            if (this.startDate && this.endDate) {
                ...
                const filtro = {
                    id_usuario: 1, // EDITAR CUANDO SE TERMINE PERFILAMIENTO
        ...
        /* SERVICIO OBTIENE LGO ARCHIVOS */
        async showDialogLogArchivos(id_emisor) {
            this.dessertsLogArchivos = [];     
            try {
                const filtroLog = {
                    f_ini: this.startDate,
                    f_fin: this.endDate,
                ...
                const listaLogArchivos = await getDetalleLogArchivoFiltro(filtroLog);
                this.showInfoMsg({message: "Se encontraron " + listaLogArchivos.length + " registros."});
                if (listaLogArchivos.length != 0) {
                    listaLogArchivos.forEach((objLogArchivos) => {
                        objLogArchivos.fecha = objLogArchivos.lar_fecha + " " + objLogArchivos.lar_hora;
                        this.dessertsLogArchivos.push(objLogArchivos);
                    });
                }
                
            } catch (error) {
                console.log(error);
                this.showErrorMsg({message: "Hubo un error al obtener los registros de Log archivos."});
            }
            
            this.dialogLogArchivos = true;

        },

        generarUrl(monitor, idEmisor, estadoConciliacion) {
            const pathname = window.location.pathname;
            const contextName = pathname.split('/')[1];
            if (!contextName) {
                return `/#/CyL/conccoavtran?monitor=${monitor}&idemisor=${idEmisor}&fini=${this.startDate}&ffin=${this.endDate}&estadoconciliacion=${estadoConciliacion}`;
            } else {
                return `/${contextName}/#/CyL/conccoavtran?monitor=${monitor}&idemisor=${idEmisor}&fini=${this.startDate}&ffin=${this.endDate}&estadoconciliacion=${estadoConciliacion}`;    
            }
        }
